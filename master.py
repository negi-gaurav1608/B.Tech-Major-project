...
def _wait_for_broker(timeout: float = 30.0) -> bool:
    """
    Poll until the Kafka broker reports at least one topic partition
    in its metadata, or until timeout. This fixes the
    'Controller ID not available, got -1' / empty-metadata race
    that occurs when ZooKeeper has not yet finished registering the
    broker after a fresh Kafka start.
    Returns True if broker is ready, False if timed out.
    """
    import socket as _sock
    host, port_str = cfg.KAFKA_BROKER.rsplit(":", 1)
    port = int(port_str)
    deadline = time.time() + timeout
    logger.info(f"Waiting for Kafka broker at {cfg.KAFKA_BROKER} …")
    while time.time() < deadline:
        # Layer 1: TCP reachable?
        try:
            ...
            time.sleep(2)
            continue
        # Layer 2: Kafka metadata ready? (controller_id == -1 means not yet)
        try:
            ...
            meta = _c._client.cluster
            _c.close()
            # controller_id of -1 means ZooKeeper registration not done
            if getattr(meta, "controller", None) not in (None, -1):
                logger.info("Kafka broker metadata ready (controller elected).")
                return True
            # some kafka-python versions expose it differently
            if hasattr(meta, '_controller_id') and meta._controller_id not in (None, -1):
                logger.info("Kafka broker metadata ready.")
                return True
            logger.info("Broker connected but metadata not ready yet — waiting …")
        except Exception:
            pass
        time.sleep(3)
    logger.warning(f"Kafka broker not fully ready after {timeout:.0f}s — continuing anyway.")
    return False


def ensure_topics(retries: int = 8, delay: float = 4.0):
    """
    Wait for broker metadata to stabilise, then create all five
    Kafka topics. Retries handle the transient 'No broker metadata'
    window between Kafka start and full ZooKeeper registration.
    """
    _wait_for_broker(timeout=40.0)
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            logger.info("Kafka topics already exist — broker is ready.")
            return
        except Exception as e:
            logger.warning(
                f"Topic creation attempt {attempt}/{retries} failed: {e}. "
                + (f"Retrying in {delay:.0f}s …" if attempt < retries
                   else "Giving up — topics may already exist or Kafka is unavailable.")
            )
            if attempt < retries:
                time.sleep(delay)
...
def frame_stats():
    """
    Returns per-slave frame processing time history and frame counts —
    consumed by the dashboard performance graphs.
    Response shape:
    {
      "slaves": {
        "<slave_id>": {
          "history":      [ms, ms, ...],   # last 100 frame times
          "total_frames": 1842,
          "avg_ms":       412.3
        },
        ...
    ...
    lb = get_load_balancer()
    return jsonify({
        "slaves":    lb.get_all_frame_time_history(),
        "timestamp": time.time(),
    })
...
if __name__ == "__main__":
    # ── Kafka: best-effort topic creation — server runs even if Kafka is down ──
    ensure_topics()

    # ── Load balancer (Kafka consumer in background thread) ───────────────────
    get_load_balancer()

    # ── Re-encode any existing mp4v files to H.264 on startup ─────────────────
    migrate_existing_jobs()

    # ── Find a free port if the configured one is busy ────────────────────────
    import socket as _socket
    port = cfg.WEB_PORT
    while True:
        with _socket.socket(_socket.AF_INET, _socket.SOCK_STREAM) as _s:
            if _s.connect_ex(("0.0.0.0", port)) != 0:
                break          # port is free
        logger.warning(f"Port {port} is already in use — trying {port + 1}")
        port += 1

    logger.info(f"Master on http://0.0.0.0:{port}")
